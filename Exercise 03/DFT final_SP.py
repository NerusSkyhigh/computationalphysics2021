...
#dE = 0.1
L = 23 # Mesh length  (for 8 electrons until 15 is fine, for 20 electrons til 17.5 is ok with 0.00005 I think )
...
def level(n,l,Ener, rho, dE):
    ...
    #print("{}".format(Ener))
    #print("{}".format(lastnow))
    if (lastnow < 0.05 and lastnow > -0.05):
        print("no refinement")
        print("{}".format(Ener))
        ...
        while (lastnow > 0.05 or lastnow < -0.05) :
            rat = 1
            lastbefore = lastnow
            u[0] = 0
            u[1] = pow(dx, l+1)
...
#for j in range(0,1):
    ...
    u1s, E[0,0],lastnow1s = level(1,0, minE , rhomix, dE)
    print("First level done")
    u1p, E[0,1],lastnow1p = level(1,1, E[0,0] , rhomix,dE)
    print("Second level done")
    u1d, E[0,2],lastnow1d = level(1,2, E[0,1], rhomix,dE)
    print("Third level done")
    u2s, E[1,0],lastnow2s = level(2,0, E[0,2] , rhomix,dE)
    print("Fourth level done")
    u1f, E[0,3],lastnow1f = level(1,3, E[0,2],  rhomix,dE)
    print("Fifth level done")
    u2p, E[1,1],lastnow2p = level(2,1, E[1,0],  rhomix,dE)
...
#plt.plot(x[1:Nx], rhoconstruct(R1s, x, 0)[1:Nx]/(rhob*Ir) *Ne, linewidth=0.9, label="1s")
#plt.plot(x[1:Nx], rhoconstruct(R1p, x, 1)[1:Nx]/(rhob*Ir) *Ne, linewidth=0.9, label="1p")
#plt.plot(x[1:Nx], rhoconstruct(R1d, x, 2)[1:Nx]/(rhob*Ir) *Ne, linewidth=0.9, label="1d")
#plt.plot(x[1:Nx], rhoconstruct(R2s, x, 0)[1:Nx]/(rhob*Ir) *Ne, linewidth=0.9, label="2s")
#plt.plot(x[1:Nx], rhoconstruct(R2p, x, 1)[1:Nx]/(rhob*Ir) *Ne, linewidth=0.9, label="2p")
#plt.plot(x[1:Nx], rhoconstruct(R1f, x, 3)[1:Nx]/(rhob*Ir) *Ne, linewidth=0.9, label="1f")
