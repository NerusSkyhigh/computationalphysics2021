...
def direct(r, rho):
    ...
    if r!=0:
        d = 4*m.pi* (np.trapz(rho * rpr1**2, rpr1)/r + np.trapz(rho * rpr2, rpr2))
    else:
        d = 4*m.pi* np.trapz(rho * rpr2, rpr2)
...
#    Ener = - 2.0 * m.pi * rhob * Rc**2+dE #set the starting energy at the bottom of the well
    ...
    u1s, E[0,0],lastnow1s = level(1,0, - 2.0 * m.pi * rhob * Rc**2+dE, 1, rhomix)
...
Ir = np.trapz(rhoinitial*x**2*4*m.pi, x )
...
pol = np.trapz(rhoinitial*xpol**2*4*m.pi, xpol)
...
fig1 = plt.figure()
plt.plot(x[1:int(L/dx)], rhoinitial[1:int(L/dx)], linewidth=0.9)
...
def drawvec():
    ...
    for i in range(0, Nx):
        V0[i] = draw_pot(x[i], rhoinitial, rhoinitial[i])
        di[i] = direct(x[i], rhoinitial)
        co[i] = -2.0 * A * (1 + a1 * rs) * m.log(1.0 + 1.0 / (2.0 * A * (b1 * rs**(1.0/2.0) + b2 * rs + b3 * rs**(3.0/2.0) + b4 * rs**(p + 1.0) ) ) )
        ex[i] = -(3/m.pi *rhoinitial[i])**(1/3)
...
plt.plot(x[2:Nx], co[2:Nx] , linewidth=0.9, color="green", label="correlation")
plt.title("Potential")
