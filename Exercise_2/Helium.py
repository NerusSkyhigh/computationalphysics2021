...
def G(x, a, b):
    return 2 * m.sqrt(2) / m.sqrt(m.pi) * m.exp(-2 * x)
...
def Gex2(x):
    return (2 * m.sqrt(2) * m.exp(-2 * x) / m.sqrt(m.pi) )**2
...
print("The norm of the exact function is {} pm {} ".format(I2[0],I2[1]))
...
def Dex(x):
    return (( 1 / m.sqrt(m.pi) ) * np.exp(-1 * x) / m.sqrt(I2[0]) )**2 
